...
Now consider the history $\mathcal{H}_t^z = (\bm{l},\, \vz_{t-H+1:t,\text{clean}},\, \va_{t-H:t-1})$, which includes:
\begin{itemize}[leftmargin=*]
  \item $\bm{l}$: the language instruction (e.g., ``pick up the red block''),
  \item $\vz_{t-H+1:t,\text{clean}}$: past observation latents,
  \item $\va_{t-H:t-1}$: past actions.
\end{itemize}
...
This is the information in the future latent $\vz_{t+1}$ that is \textbf{shared with the history and language instruction}.
...
\begin{itemize}[leftmargin=*]
  ...
  \item The last action $\va_{t-1}$ moved the gripper downward toward the block.
\end{itemize}
Then the fact that ``the gripper is now in contact with the red block'' in $\vz_{t+1}$ is \textbf{predictable from the history and language goal}. A good dynamics model, knowing the task goal and the approach trajectory, could predict this. This is term (A).

\textbf{Why it is ``free'':} A perfect dynamics prior, given the history and language, could predict this part of $\vz_{t+1}$ with zero error. The prior loss does not penalize this information because the prior can match it.

\subsection{Term (B): Unpredictable Information --- $I(\vz_{t+1};\, \vo_{t+1} \mid \mathcal{H}_t^z)$}

This is the information in $\vz_{t+1}$ about $\vo_{t+1}$ that \textbf{cannot be explained by the history or language}.
...
\end{itemize}
None of these changes can be predicted from the robot's past observations, actions, or the language instruction ``pick up the red block.'' If the encoder encodes these details into $\vz_{t+1,\text{clean}}$, that information is \emph{new}---it was not in $\mathcal{H}_t^z$. This is term (B).

\textbf{Why it is ``penalized'':} The prior, no matter how powerful, cannot predict this from the history and language. So the prior's prediction $\hat{\vz}_{t+1}$ will differ from $\vz_{t+1,\text{clean}}$ on exactly these components, creating a nonzero MSE.
...
This is a standard result. For any two distributions $q(\vz \mid \vo)$ (encoder) and $p_\theta(\vz \mid \mathcal{H})$ (prior, conditioned on history + language):
\begin{align}
...
\textbf{Intuition:} The KL measures how badly the prior $p_\theta$ predicts the encoder's output. This is at least as large as the ``intrinsic unpredictability'' (the conditional mutual information), plus any ``modeling error'' from the prior not being perfect.

\textbf{When does equality hold?} When $p_\theta(\vz \mid \mathcal{H}) = q(\vz \mid \mathcal{H})$, i.e., the prior perfectly matches the true conditional distribution of latents given history and language. In that case, the KL exactly equals the mutual information.
...
\begin{itemize}[leftmargin=*]
  ...
  \item The encoder achieves this by not encoding things that the history and language cannot explain (shadows, noise, background changes).
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item The prior gets better at predicting the latent from history and language.
  \item But this does not change the ``intrinsic unpredictability'' $I(\vz; \vo \mid \mathcal{H})$---it only reduces the modeling gap.
\end{itemize}

\section{Role of the Action Loss}

The latent prior loss alone would push the encoder to discard unpredictable information. But what prevents the encoder from also discarding \emph{predictable} action-relevant information?

The action loss $\loss_a = \|\va_t - \hat{\va}_t\|^2$ adds an inverse dynamics signal: the prior must also recover the action $\va_t$ that \emph{caused} the transition $\vo_t \to \vo_{t+1}$.

This means the latent $\vz_{t+1,\text{clean}}$ must encode the causal effect of $\va_t$: if the action closed the gripper on the red block (as instructed by $\bm{l}$), the latent must show the contact geometry precisely enough to recover this action. Without the action loss, the encoder could collapse to something trivially predictable (e.g., a constant) that the prior matches perfectly but carries no useful information.

Together, the three losses create the equilibrium:
\begin{equation}
...
\begin{itemize}[leftmargin=*]
  \item 70 bits are about gripper/object geometry (predictable from history and language).
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Keep encoding them and hope the prior learns to predict them} $\to$ impossible, because they are genuinely random (not a function of the history or language).
\end{enumerate}
...
And the action loss $\loss_a$ ensures that among the retained bits, the \emph{action-causal} geometry is prioritized: the latent must encode enough about the transition $\vz_t \to \vz_{t+1}$ to recover the action $\va_t$ that caused it.
