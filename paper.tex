...
Given a history of observations $\vo_{t-H+1:t}$, past actions $\va_{t-H:t-1}$, and a language instruction $\bm{l}$, our dynamics prior \emph{jointly} generates the current action $\va_t$ and the next observation latent $\vz_{t+1}$---the frame that results from executing $\va_t$---unifying world modeling and policy learning in a single generative process.
By conditioning the diffusion prior on the observation-action history and language goal, the variational bound \emph{mathematically forces} the encoder to discard unpredictable visual content while retaining the physical geometry that the history of interactions can predict.
...
The key insight is that by conditioning the diffusion prior on the history of past observations, past actions, and a language instruction, and requiring it to jointly predict the current action $\va_t$ \emph{and} the resulting next observation latent $\vz_{t+1}$, any encoder information that cannot be predicted from this context will spike the variational loss.
The encoder is thus incentivized to discard unpredictable pixels and retain only the physical geometry that the robot's interaction history and task goal can explain.

This formulation---where the model takes in $(\vo_{t-H+1:t}, \va_{t-H:t-1}, \bm{l})$ and generates $(\va_t, \vo_{t+1})$, with $\vo_{t+1}$ being the consequence of executing $\va_t$---follows the autoregressive structure of DreamZero~\citep{dreamzero2025}, but operates in a principled latent space rather than raw pixels.
...
The central idea is to replace the unconditional prior $p_\theta(\vz_0)$ with a \emph{dynamics prior} $p_\theta(\va_t, \vz_{t+1,0} \mid \mathcal{H}_t, \bm{l})$ that must jointly predict the current action and the resulting future observation latent, conditioned on the interaction history and a language instruction.
...
\begin{equation}
  \mathcal{H}_t^z = \big(\bm{l},\; \vz_{t-H+1,\text{clean}}, \va_{t-H}, \vz_{t-H+2,\text{clean}}, \va_{t-H+1}, \ldots, \va_{t-1}, \vz_{t,\text{clean}}\big).
...
We replace this with a \emph{joint dynamics prior} that conditions on the full interaction history $\mathcal{H}_t^z$ (including language instruction) and simultaneously predicts the current action and the resulting future observation latent:
\begin{equation}
  \big(\hat{\va}_t,\; \hat{\vz}_{t+1,\text{clean}}\big) = \hat{f}_\theta^{\text{dyn}}\!\left(\vz_{t+1,\tau},\; \mathcal{H}_t^z\right).
  \label{eq:dynamics_prior_pred}
\end{equation}

The prior predicts:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{Current action:} $\hat{\va}_t \approx \va_t$ --- what action to take now, given the history and language goal.
  \item \textbf{Future latent:} $\hat{\vz}_{t+1,\text{clean}} \approx \vz_{t+1,\text{clean}}$ --- the observation that results from executing $\va_t$.
\end{enumerate}
The action and future frame are causally linked: $\va_t$ is the action that \emph{causes} the transition $\vo_t \to \vo_{t+1}$. The model must generate both consistently.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Random textures/lighting:} If $E_\theta$ preserves random background variations, the dynamics prior cannot predict these from the history (past frames, past actions, and the language goal do not determine future lighting). The latent MSE spikes.
  ...
  \item \textbf{Action-caused geometry:} Gripper position, object displacement, and contact geometry \emph{are} predictable from the history via learned physics. The prior can reason: ``given the language goal `pick up red block', the history of gripper approach, and the predicted action $\va_t$ (close gripper), the next frame should show the gripper grasping the block.'' Encoding this geometry \emph{decreases} both MSE terms.
\end{enumerate}

The \emph{joint} prediction of $(\va_t, \vz_{t+1})$ creates two complementary pressures:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Forward dynamics:} The prior must predict $\vz_{t+1}$ --- what will happen after executing $\va_t$. This requires the latent to encode physical geometry that actions causally affect.
  \item \textbf{Inverse dynamics:} The prior must also predict $\va_t$ --- what action caused the transition $\vz_t \to \vz_{t+1}$. This prevents the encoder from collapsing to a trivially predictable but uninformative representation (e.g., encoding a constant). The latent transition must be informative enough to recover the action.
\end{itemize}
...
\end{equation}
The prior loss $\loss_z^{\text{latent}}$ upper-bounds the first term (see \Cref{sec:theory} for proof). The decoder loss $\loss_x$ incentivizes maximizing total information. The action loss $\loss_a$ ensures the latent captures the causal effect of actions (inverse dynamics signal). Together, they push the encoder toward a representation that is maximally informative about action-predictable physics while discarding everything else.
...
\STATE Construct latent history: $\mathcal{H}_t^z = (\bm{l}, \vz_{t-H+1,\text{clean}}, \va_{t-H}, \ldots, \va_{t-1}, \vz_{t,\text{clean}})$
...
A causal Vision Transformer (ViT) with 12 blocks and 768 hidden dimensions. The input sequence is constructed as language tokens followed by interleaved observation-action history:
\begin{equation}
  \big[\text{text\_enc}(\bm{l}),\; \text{patchify}(\vz_{t-H+1,\text{clean}}),\; \text{mlp}(\va_{t-H}),\; \ldots,\; \text{mlp}(\va_{t-1}),\; \text{patchify}(\vz_{t,\text{clean}}),\; \text{patchify}(\vz_{t+1,\tau})\big].
  \label{eq:prior_input}
\end{equation}
The language instruction $\bm{l}$ is encoded by a frozen text encoder into a sequence of tokens prepended to the history. Each latent $\vz_{k,\text{clean}}$ is patchified into spatial tokens; each action $\va_k$ is projected through a 2-layer MLP into a single token. The diffusion timestep $\tau$ is encoded via sinusoidal embeddings and added to the noisy future latent tokens.
...
During deployment, the model operates in a closed loop. At each step $t$, the history $\mathcal{H}_t^z$ contains the language instruction, past latents $\vz_{t-H+1:t,\text{clean}}$, and past actions $\va_{t-H:t-1}$:

\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{Encode:} $\vz_{t,\text{clean}} = E_\theta(\vo_t)$ --- encode the current observation with the frozen encoder.
  \item \textbf{Generate:} $(\hat{\va}_t, \hat{\vz}_{t+1,\text{clean}}) = \mathcal{T}_\phi(\mathcal{H}_t^z)$ --- jointly generate the action to take now and the resulting next latent via iterative denoising.
  ...
  \item \textbf{Replace:} Compute $\vz_{t+1,\text{clean}} = E_\theta(\vo_{t+1})$ and replace $\hat{\vz}_{t+1,\text{clean}}$ in the KV cache with this ground-truth encoding. This prevents error accumulation.
  \item \textbf{Advance:} Shift the history window: $\va_t$ becomes part of the past actions, $\vz_{t+1,\text{clean}}$ becomes the latest observation latent. Go to step 2 with $t \leftarrow t+1$.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \begin{equation}
    (\hat{\va}_t, \hat{\vz}_{t+1}) \to (\hat{\va}_{t+1}, \hat{\vz}_{t+2}) \to \cdots \to (\hat{\va}_{t+P-1}, \hat{\vz}_{t+P}).
    \label{eq:open_loop_rollout}
  \end{equation}
  At each step, the predicted $\hat{\vz}$ is fed back as context for the next step.
...
\begin{equation}
  -\log p_\theta(\va_t, \vo_{t+1} \mid \mathcal{H}_t) \leq \loss_z^{\text{latent}}(\theta) + \gamma \cdot \loss_a(\theta) + c_{\text{lf}} \cdot \loss_x(\theta),
  \label{eq:acul_bound}
\end{equation}
...
We introduced Action-Conditioned Unified Latents (AC-UL), a framework for learning visual representations optimized to encode only the physical geometry predictable from the history of observations, actions, and language instructions.
By extending the Unified Latents variational objective with a history-conditioned joint dynamics prior that simultaneously generates the current action and the resulting future observation, we create an information bottleneck that discards unpredictable visual content while retaining action-relevant structure.

Our formulation follows the autoregressive paradigm of DreamZero---where each step generates an action $\va_t$ and the frame $\vo_{t+1}$ it causes---but operates in a principled latent space rather than raw pixels.
Stage~1 trains the physics-grounded encoder via the AC-UL objective, and Stage~2 trains an autoregressive diffusion transformer on frozen latents for joint action-observation generation in closed loop.
...
We model the joint distribution $p_\theta(\va_t, \vo_{t+1} \mid \mathcal{H}_t)$, where $\mathcal{H}_t = (\bm{l}, \vo_{t-H+1:t}, \va_{t-H:t-1})$ includes the language instruction. Introducing a latent $\vz_{t+1,0}$:
\begin{align}
...
The conditioning on $\mathcal{H}_t^z$ is the key difference from standard UL: the prior uses the full interaction history---language instruction, past observation latents, and past actions---to predict the future. The action $\va_t$ being predicted is the one that \emph{causes} the transition to $\vo_{t+1}$, so the model must generate both consistently.
